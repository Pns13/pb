...
def menu(my_name,my_id):
	...
	alvino_xy(f'忠➥➣ID  : '+str(my_id))
	alvino_xy(f'忠➥➣IP  : {ip}')
	cetak(nel('\t[bold cyan]                 • MENU CRACK • [/bold cyan]'))
	print('')
	cetak(nel('{m}➣1. Crack Publik 
                       {k}➣2. Crack Follower 
	                   {h}➣3. Crack Grup 
	                   {u}➣4. Crack File	
	                   {b}➣5. Hasil Crack ')) 
	print('➥➥➥➣0. Keluar [bold cyan]      ')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]                  • TIPE AKUN • [/bold cyan]'))
	print('') 
	cetak(nel('{m}➣1. Akun Old 
                       {k}➣2. Akun New 
                       {h}➣3. Random [bold cyan]'))
	print('')
	hu = input('➥➥➣Pilih : ')
	if hu in ['1','01']:
...
def passwrd():
	...
	print('')
	print(f'➥➥➣Hasil {h}OK{u} Tersimpan Di : {h}OK/%s {h}'%(okc))
	print(f'➥➥➣Hasil {k}CP{h} Tersimpan Di : {k}CP/%s {k}'%(cpc))
